...
| **R-T1.2 폴더 개요 (v1.2.55)** | 최내부 ENGINE wrapper 안에 URL 도메인 트리를 **root→대→중→소 full-depth 폴더 subgraph로 중첩** emit (`buildNestedFolderOverviewLines`). 폴더 헤더 라벨 = `📁 /name · N routes`(하위 route 재귀 합 배지). 구조적 폴더 = `subgraph`, leaf-folder(자식 전부 terminal)·terminal = 카운트 박스(개별 route 노드로 펼치지 않음 — Tab2 위임). top-level 형제는 단일 `~~~` chain으로 X축 분포(render-check 채택, §10), nested 자식은 Y-stack. 노드 수는 폴더 수준이라 단일 다이어그램 유지(R-T1.7). **(v1.2: top-level 도메인 박스 1개씩 `buildDomainSummaryLines` — §10에서 폴더 개요로 재정의)** |
...
- **v1.2.55** (2026-06-23, v1.2.55 사이클 채택): **Tab1 도메인 요약 → full-depth 폴더 개요로 재정의**(R-T1.2 v1.2.55, `buildNestedFolderOverviewLines`). top-level 박스 1개로는 대형 다도메인 디렉터리 구조가 안 보이던 한계 + v1.2.53 청킹 폐지가 `buildDiagrams` rendering 필드에서 누수(routeCount>300 재청킹)되어 wrapper 반복·도메인 누락 체감 결함 재발한 것을 해소(ST0 직접 호출 교체). 레이아웃은 render-check 실측으로 top-level `~~~` chain 채택(row-wrapper는 mermaid v11 무시 재확인). 본문 §2.2 R-T1.2 정정. 자세한 사유·실측은 §10 참조.
...
## 10. v1.2.55 Re-amendment 본문 — Tab1 도메인 요약 → full-depth 폴더 개요

> **상태**: 채택 (2026-06-23, v1.2.55 사이클)
> **연관**: [[project_v1255_tab1_tab2_redesign]] · `scripts/tab1-render-check.mjs` · [[feedback_mermaid_v11_nested_lr_limit]]
> **사유**: v1.2 도메인 요약(top-level 박스 1개씩)은 대형 다도메인(WINA 516 routes·16 도메인)에서 도메인 내부 구조가 안 보여 "디렉터리 구조" 요구를 충족 못 함. 또한 v1.2.53 청킹 폐지(§9)가 `buildDiagrams`(mermaid-renderer.ts)에서 누수되어(rendering이 여전히 `buildWithChunkFallback`·routeCount>300 게이트) wrapper 반복·도메인 누락 체감 결함 재발.

### 10.1 결정

**Tab1은 top-level 도메인 박스 1개가 아니라, 단일 아키텍처 래퍼 안에 URL 도메인 트리를 root→대→중→소 full-depth 폴더 subgraph로 중첩한다. 각 폴더 헤더는 하위 route 재귀 합 배지를 표시하고, 최하위 route 세그먼트(leaf-folder: 자식이 전부 terminal)는 개별 route 노드로 펼치지 않고 카운트 박스로 collapse한다.**

- emitter: `buildNestedFolderOverviewLines`(`fe/tab1-tree.ts`) — `buildDomainSummaryLines`(v1.2) 대체.
- 구조적 폴더(하위 분기 존재) → `subgraph`, leaf-folder·terminal → 카운트 박스(`📁 /name · N routes`).
- **개별 route URL 전수 열거는 여전히 Tab2 위임**(탭 차별성 유지). v1.2.53의 "하위 세분화 Tab2 위임"을 **폴더 구조 한정으로 완화**(route 단위는 계속 Tab2).
- 누락 0: top-level 도메인 전수 출력 + 재귀 카운트로 보장(검증 ST5 실 516).

### 10.2 레이아웃 (render-check 실측)

`scripts/tab1-render-check.mjs`로 WINA 규모(16 도메인·~291 page route) 실제 mermaid 렌더 비교:

| 전략 | 크기 | aspect | 판정 |
|---|---|---|---|
| bare (no chain) | 991×27571 | 0.04 | ❌ 세로 strip |
| **top-level `~~~` chain** | **8522×4363** | **1.95** | ✅ 채택 |
| row-wrapper grid | 1291×27761 | 0.05 | ❌ 무시됨([[feedback_mermaid_v11_nested_lr_limit]] 재확인) |
| outer graph TD | 54685×874 | 62 | ❌ 가로 strip |

→ **top-level 형제는 단일 `~~~` chain으로 X축 분포**(R-T1.2 v1.1 유지), nested 자식은 Y-stack. row-wrapper(emitInnerRowSubgraphs)는 v1.2 코드에 있었으나 mermaid v11이 시각적으로 무시함이 실측 재확인되어 폐기.
